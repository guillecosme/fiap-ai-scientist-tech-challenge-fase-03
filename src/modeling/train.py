...
from src.preprocessing.splits import SEED, amostra_estratificada, cv_por_municipio
...
def treinar_municipio(n_iter: int = 20, modelo: str = "lightgbm") -> dict:
    ...
    busca = buscar_hiperparametros(
        pipeline, distribuicoes, x_tr, y_tr, treino["id_municipio"], n_iter=n_iter, n_jobs=-1
    )
    final = busca.best_estimator_
    proba = final.predict_proba(x_te)[:, 1]
    metricas = avaliar_classificacao(y_te.values, proba)
    salvar_modelo(
        final,
        "modelo_municipio",
        {
            ...
            "auc_validacao": float(busca.best_score_),
            "metricas_teste_2025": metricas,
            "features": grupos,
        },
    )
    print(
        f"[treino] municipio ({modelo}): AUC validacao {busca.best_score_:.3f} "
